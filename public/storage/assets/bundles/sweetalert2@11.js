...
`;
    ...
    const setInnerHtml = (elem, html) => {
        ...
        if (html) {
            ...
            Array.from(parsed.querySelector("head").childNodes).forEach(
                (child) => {
                    elem.appendChild(child);
                }
            );
            Array.from(parsed.querySelector("body").childNodes).forEach(
                (child) => {
                    if (
    ...
    const animateTimerProgressBar = function (timer) {
        ...
        if (isVisible$1(timerProgressBar)) {
            ...
            setTimeout(() => {
                timerProgressBar.style.transition = `width ${
                    timer / 1000
                }s linear`;
...
`;
    ...
    const addClasses$1 = (popup, params) => {
        ...
        popup.className = `${swalClasses.popup} ${
            isVisible$1(popup) ? showClass.popup : ""
        }`;
        if (params.toast) {
            addClass(
                [document.documentElement, document.body],
